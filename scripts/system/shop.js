...
import { tagback, ForceOpen, getScore, metricNumbers } from "../lib/game"
import { itemBlock, itemBlockColor, itemLog, itemFurniture, itemGlass, itemArmor, itemFarm, itemFood, itemOres, itemSpawner, itemSword, itemPickaxe, itemAxe, itemShovel } from "../shop"
system.runInterval(() => tagback('shop', ShopSystem))
...
export async function ShopSystem(player) {
  ...
  if (result.selection === 0) buySell(player, itemBlock)
  if (result.selection === 1) buySell(player, itemBlockColor)
  if (result.selection === 2)  buySell(player, itemLog)
  if (result.selection === 3) buySell(player, itemFurniture)
  if (result.selection === 4) buySell(player, itemGlass)
  if (result.selection === 5) Tools(player)
  if (result.selection === 6) buySell(player, itemArmor)
  if (result.selection === 7) buySell(player, itemFarm)
  if (result.selection === 8) buySell(player, itemFood)
  if (result.selection === 9) buySell(player, itemOres)
  if (result.selection === 10) buySell(player, itemSpawner)
}
...
function Tools(player) {
  ...
  .show(player).then(result => {
    ...
    if (result.selection == 0) buySell(player, itemSword)
    if (result.selection == 1) buySell(player, itemAxe)
    if (result.selection == 2) buySell(player, itemPickaxe)
    if (result.selection == 3) buySell(player, itemShovel)
    if (result.selection == 4) Shop(player)
  })
}
...
function buySell(player, itemName) {
  ...
  .title(`BuySell | Shop`)
  itemName.forEach((v) => gui.button(`§l${v.name}\n§r§7Cost: §g${v.cost}`, `${v.textures}`))
  gui.button("§l§cBACK\n§r§oClick To Back")
  gui.show(player).then(result => {
  if (result.canceled) return
  if (itemName[result.selection]) {
    ...
    brick.show(player).then(res => {
      ...
      if (res.formValues[0].startsWith("-") || res.formValues.startsWith("+")) return player.sendMessage(`§cCannot be prefixed with ${res.formValues[0].slice(0, 1)}, must fill in with numbers`) && player.playSound(`note.bass`)
